...
import 'package:lexa/presentation/screens/create_topic_screen.dart';
import 'package:lexa/data/models/topic.model.dart';
import 'package:lexa/presentation/views/create_folder_dialog.dart';
import 'package:lexa/presentation/views/topic_grid_item.dart';
import 'package:lexa/presentation/views/topic_list_item.dart';
import 'package:lexa/presentation/views/back_app_bar.dart';
import 'package:lexa/presentation/views/confirm_dialog.dart';
import 'package:lexa/presentation/views/list_order_mode_toggle_button.dart';
...
class ManageTopicScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _ManageTopicScreenContent(
      rootId: rootId,
      folderId: folderId,
...
class _ManageTopicScreenContent extends StatefulWidget {
  final String? rootId;
  final String? folderId;
  final String? folderName;

  const _ManageTopicScreenContent({
    Key? key,
    this.folderId,
    this.folderName,
    this.rootId,
  ...
  @override
  State<_ManageTopicScreenContent> createState() =>
      _ManageTopicScreenContentState();
}

class _ManageTopicScreenContentState extends State<_ManageTopicScreenContent> {
  ...
  Widget _buildScaffold(BuildContext context, ManageTopicState state) {
    ...
    return Scaffold(
      ...
      body: BlocBuilder<CreateTopicBloc, CreateTopicState>(
              ...
              child: isListMode
                  ? ListView.builder(
                      ...
                      itemBuilder: (context, index) =>
                          _buildListItem(context, node.children[index], index),
                    )
                  : Padding(
                      key: UniqueKey(),
                      ...
                      child: GridView.builder(
                        ...
                        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                          crossAxisCount:
                              (MediaQuery.of(context).size.width / 110).round(),
                          crossAxisSpacing: 10,
                          mainAxisSpacing: 10,
                        ),
                        itemBuilder: (context, index) {
                          if (index < node.children.length) {
                            return _buildGridItem(
                                context, node.children[index], index);
                          }
                          return Column(
                            children: [
                              Stack(
  ...
  void _handleDrop(dynamic dropData, DragTargetDetails<Object?> target) {
    ...
    if (dropData is Folder &&
        (data is Folder ? data.id != dropData.id : true)) {
      showDialog(
        context: context,
    ...
    } else if ((dropData is String || dropData == null) &&
