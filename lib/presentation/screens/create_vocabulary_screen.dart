...
import 'package:lexa/presentation/views/adding_vocabulary_list.dart';
import 'package:lexa/presentation/views/create_answer_dialog.dart';
import 'package:lexa/presentation/views/create_answers_segment.dart';
import 'package:lexa/presentation/views/styled_dropdown_button.dart';
...
class _CreateVocabularyPageState extends State<CreateVocabularyPage> {
  ...
  void initState() {
    ...
    _debouncedVocabulary = DebounceUtils.debounce((text) {
      final updatingVocabularies =
          _createTopicBloc.state.topic.data.vocabularies;
      updatingVocabularies[activeTab].word = text;

      _createTopicBloc.add(
        UpdateNewTopic(
          topic: _createTopicBloc.state.topic.copyWith(
            data: _createTopicBloc.state.topic.data
                .copyWith(vocabularies: updatingVocabularies),
          ),
        ),
      );
    }, 300);
      ...
      builder: (context, state) {
        return Scaffold(
          ...
          body: PageView(
            ...
            children: state.topic.data.vocabularies
                .map((vocabulary) => _buildBody(vocabulary, state))
                .toList(),
          ),
        ...
        children: [
          Padding(
            ...
            child: ThumbnailPickerSegment(
              ...
              onPicked: (image) {
                ...
                _updateTopic(state.topic.copyWith(
                    data: state.topic.data
                        .copyWith(vocabularies: updatedVocabularies)));
              },
            ),
          ),
          ...
          TextField(
            ...
            decoration: InputDecoration(
              hintText: state.topic.data.vocabularies[activeTab].word ??
          ...
          CreateAnswerSegment(
            answers:
                state.topic.data.vocabularies[activeTab].multipleChoiceAnswers,
  ...
  void _onAddVocabulary(CreateTopicState state) {
    final validationResult =
        state.topic.data.vocabularies[activeTab].validate();
    ...
    showDialog(
      ...
      builder: (context) => ConfirmDialog(
        action: () {
          ...
          _updateTopic(_createTopicBloc.state.topic.copyWith(
              data: _createTopicBloc.state.topic.data
                  .copyWith(vocabularies: updatingVocabularies)));
        },
  ...
  void _updateAnswerCount(CreateTopicState state, int count) {
    ...
    if (count < updatingVocabularies[activeTab].multipleChoiceAnswers.length) {
      updatingVocabularies[activeTab].multipleChoiceAnswers =
          updatingVocabularies[activeTab]
              .multipleChoiceAnswers
              .sublist(0, count);
    } else {
      updatingVocabularies[activeTab]
          .multipleChoiceAnswers
          .add(CreateMultipleChoiceAnswerDto());
    }

    _updateTopic(state.topic.copyWith(
        data: state.topic.data.copyWith(vocabularies: updatingVocabularies)));
  }
  ...
  void _showCreateAnswerDialog(
      BuildContext context, CreateTopicState state, int index) {
    showDialog(
      context: context,
      builder: (context) => CreateAnswerDialog(
        color: multipleChoiceAnswerColors[index]!,
        answer: state
            .topic.data.vocabularies[activeTab].multipleChoiceAnswers[index],
        onChanged: (answer) {
